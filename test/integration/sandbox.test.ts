...
describe.skipIf(!apiLogin || !apiPassword)('DskVposClient against the real UAT sandbox', () => {
  ...
  it('survives a burst of concurrent orders with unique ids', async () => {
    const results = await Promise.all(
      Array.from({ length: 15 }, (_, i) =>
        client.registerOrder({ orderNumber: `sdk-test-burst-${Date.now()}-${i}`, amountCents: 100, currency: '978', returnUrl: 'https://example.com/return' })
      )
    );
  ...
  it('reports a short timeout as a DskVposError and recovers on the next call', async () => {
    const impatient = new DskVposClient({ apiLogin: apiLogin ?? '', apiPassword: apiPassword ?? '', environment: 'uat', timeoutMs: 1 });
    await expect(
      impatient.registerOrder({ orderNumber: `sdk-test-timeout-${Date.now()}`, amountCents: 100, currency: '978', returnUrl: 'https://example.com/return' })
    ).rejects.toThrow(DskVposError);

    const ok = await client.registerOrder({ orderNumber: `sdk-test-recover-${Date.now()}`, amountCents: 100, currency: '978', returnUrl: 'https://example.com/return' });
